...
outlier sentinels. Our next evaluation layer tests whether the same evidence
survives retrieval-conditioned generation under OCR, outlier, and density-based
filtering attacks.
\end{abstract}
...
The next experiments add a minimal RAIG layer on top of the COCO retrieval
evidence. Baselines include ImageSentinel-style visible key sentinels,
conventional invisible watermarking, and no-sentinel controls. Attacks include
OCR/text filtering, CLIP outlier filtering, local-density filtering, and
deduplication. Metrics are ownership detection true-positive rate at fixed
false-positive rate, sentinel survival, normal collateral damage, trigger
retrieval hit rate, generation-side evidence, and non-trigger RAIG utility.
